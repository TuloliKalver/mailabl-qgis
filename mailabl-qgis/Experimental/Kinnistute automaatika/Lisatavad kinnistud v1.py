...
for feature in input_layer[0].getFeatures():
    ...
    if REGISTR_date:
        REGISTR_date = QDate.fromString(REGISTR_date.toString("yyyy-MM-dd"), "yyyy-MM-dd")
        if not latest_date or REGISTR_date > latest_date:
            latest_date = REGISTR_date
...
for feature in update_layer[0].getFeatures():
    REGISTR_date = feature.attribute("REGISTR")
    if REGISTR_date and REGISTR_date.toString("yyyy-MM-dd") >= latest_date_str:
